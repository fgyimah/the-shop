...
export default function cartReducer(state = initialState, action: AnyAction) {
	switch (action.type) {
		...
		case CartActionTypes.REMOVE_FROM_CART:
			...
			const itemIndex = newItems.findIndex((item) => item.product.id === action.payload.product.id);
		...
		case CartActionTypes.CLEAR_FROM_CART:
			...
			const index = cartItems.findIndex((item) => item.product.id === action.payload.product.id);
